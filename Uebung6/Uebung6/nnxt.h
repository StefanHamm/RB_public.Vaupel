...
sensor_error_t Motor_Tacho_GetCounter(const motorport_t port, uint32_t *counter){
    ...
    recvMsg("EV",msg,100);
    if (msg[0]=='E' && msg[1]=='V'){
        uint32_t dist = strtol(&(msg[2]),NULL,16);
        *counter = dist;
        return sensor_error_NoError;
    }
    else {
       printf("Fehler: EV: keine Antwort vom Server\n");
       exit(1);
    }
}
...
sensor_error_t SensorADCWithFilter(sensorport_t port, uint16_t *value){
    ...
    //printf("IRV gesendet, warte auf Ergebnis ...\n");
    recvMsg("IRV",msg,100);
    if (msg[0]=='I' && msg[1]=='R' && msg[2]=='V'){
        int dist = strtol(&(msg[3]),NULL,16);
        *value = dist;
        return sensor_error_NoError;
    }
    else {
       printf("Fehler: IRV: keine Antwort vom Server\n");
       exit(1);
    }
...
void SensorADCWithOutFilter( sensorport_t port, uint16_t *value){
    SensorADCWithFilter(port, value);
}
